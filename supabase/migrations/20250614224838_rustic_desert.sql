/*
  # Correction complète de la structure du framework
  
  1. Suppression complète des données existantes
  2. Implémentation exacte du framework JSON fourni
  3. 8 domaines, 17 macro-processus, 74 processus
  
  Structure exacte selon le JSON :
  - Plan strategy (2 macro-processus, 5 processus)
  - Govern technology (3 macro-processus, 11 processus) 
  - Manage architecture (2 macro-processus, 8 processus)
  - Manage demand (2 macro-processus, 6 processus)
  - Deliver solutions (2 macro-processus, 12 processus)
  - Operate solutions (2 macro-processus, 10 processus)
  - Support users (1 macro-processus, 6 processus)
  - Steer Resources (4 macro-processus, 16 processus)
*/

-- Suppression complète des données existantes (ordre important)
DELETE FROM scores;
DELETE FROM processes;
DELETE FROM categories;
DELETE FROM domains;

-- Insertion des 8 domaines selon le JSON
...
-- Insertion des 17 macro-processus (catégories) selon le JSON
...
-- Mise à jour des niveaux de maturité par défaut
UPDATE processes 
...
BEGIN
  ...
  RAISE NOTICE 'Structure du framework mise à jour:';
  RAISE NOTICE '- Domaines: % (attendu: 8)', domain_count;
  RAISE NOTICE '- Macro-processus: % (attendu: 17)', category_count;
  RAISE NOTICE '- Processus: % (attendu: 74)', process_count;
  
  IF domain_count != 8 OR category_count != 17 OR process_count != 74 THEN
    RAISE EXCEPTION 'Erreur: La structure ne correspond pas aux attentes!';
  END IF;
