/*
  # Nettoyage complet et implémentation exacte du framework JSON
  
  1. Nettoyage des données
    - Suppression de toutes les données des tables spécifiées
    - Suppression complète des domaines, catégories et processus existants
    
  2. Implémentation exacte du JSON
    - 8 domaines (Plan strategy, Govern technology, Manage architecture, Manage demand, Deliver solutions, Operate solutions, Support users, Steer Resources)
    - 18 macro-processus (catégories)
    - 74 processus individuels
    
  3. Structure hiérarchique exacte selon le JSON
    - Niveau Haut : 3 domaines, 7 macro-processus, 24 processus
    - Niveau Milieu : 4 domaines, 7 macro-processus, 34 processus  
    - Niveau Bas : 1 domaine, 4 macro-processus, 16 processus
*/

-- Nettoyage complet des données (ordre important pour respecter les contraintes)
DELETE FROM domain_scores;
DELETE FROM scores;
...
-- Suppression complète de la structure existante
DELETE FROM processes;
DELETE FROM categories;
DELETE FROM domains;

-- Insertion des 8 domaines selon le JSON exact
INSERT INTO domains (id, title, description, order_index, created_at, updated_at) VALUES
  -- Niveau Haut (3 domaines)
...
INSERT INTO categories (id, domain_id, title, order_index, created_at, updated_at) VALUES
  ...
  -- Manage architecture (2 macro-processus)
  ...
  -- Manage demand (2 macro-processus)
  ...
  -- Deliver solutions (2 macro-processus)
  ...
  -- Operate solutions (2 macro-processus)
  ...
  -- Support users (1 macro-processus)
  ('support-improvement', 'support-users', 'SUPPORT & IMPROVEMENT', 1, now(), now()),
  
  -- Steer Resources (4 macro-processus)
...
-- Insertion des 74 processus selon le JSON exact
INSERT INTO processes (id, category_id, name, order_index, created_at, updated_at) VALUES
  ...
  ('govern-technology-strategy', 'technology-strategy', 'Govern technology', 2, now(), now()),
...
-- Mise à jour des niveaux de maturité par défaut pour tous les processus
UPDATE processes 
...
BEGIN
  ...
  RAISE NOTICE 'Structure du framework mise à jour selon le JSON:';
  RAISE NOTICE '- Domaines: % (attendu: 8)', domain_count;
  RAISE NOTICE '- Macro-processus: % (attendu: 18)', category_count;
  RAISE NOTICE '- Processus: % (attendu: 74)', process_count;
  
  IF domain_count != 8 OR category_count != 18 OR process_count != 74 THEN
    RAISE EXCEPTION 'Erreur: La structure ne correspond pas aux attentes du JSON!';
  END IF;
  
  RAISE NOTICE 'Structure du framework correctement implémentée selon le JSON fourni!';
END $$;